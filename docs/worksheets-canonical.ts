// ============================================================================
// CANONICAL WORKSHEETS DATA — source of truth for the ExpatHub-Web repo to sync.
//
// Exported from the mobile repo (src/data/worksheets.ts) on 2026-05-16.
// If the web repo's seeded worksheet data diverges from this, the mobile
// client's submit will silently produce wrong dimensionScores or fail
// validation. Sync your `WORKSHEETS` constant and seed routine to match.
//
// Includes:
//   - WORKSHEETS array (8 worksheets, one per quiz dimension Q1–Q8)
//   - scoreWorksheet() — server-side scoring formula
//   - validateAnswersShape() — body validation for the submit route
//
// Submit body the mobile client sends:
//   POST /api/worksheets/:id/submit
//   { "answers": { "<question.id>": string | number, ... } }
//   - For `type: "scale"` questions: value is a number 1..5.
//   - For `type: "choice"` questions: value is the chosen option's `value` string.
//
// Submit response the mobile client expects on success (HTTP 200):
//   { "ok": true, "worksheetId": "<id>", "questionId": <1..8>, "dimensionScore": <0..3 float> }
//
// dimensionScore formula:
//   normalized_i = (type === "scale") ? (value - 1) / 4 : option.score
//   dimensionScore = 3 * sum(normalized_i * weight_i) / sum(weight_i)
//   clamped to [0, 3], rounded to 2 decimals.
// ============================================================================
