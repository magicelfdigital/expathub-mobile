...
- **UI Design**: Custom component library with a design token system (`theme/tokens.ts` sourcing from `constants/colors.ts`) and `StyleSheet.create` for styling. v1.1 color system: blue (#3E81DD primary), teal (#33C4DC active tabs/accents), gold (#E8991A upgrade CTAs), navy (#1C2B5E text), cream (#EEEEE4 app bg), surface (#FFFFFF cards). Fonts: Lora 600 for display headlines, DM Sans for all UI text (400/500/600/700). Active tab pills: teal filled with borderRadius 20. Cards borderRadius 14. Pathfinder badge on detail sub-screens (gold on goldLight). Map hero image on home screen.
- **Navigation**: Tab-based layout (Home, Explore, Community). Home tab is a Stack navigator (`app/(tabs)/(home)/`) containing the home screen, country list, and all country detail sub-pages (`country/[slug]/`). Tapping a country card from Home does `router.push` within the Home stack, creating a fresh screen each time (no cross-tab navigation). Subscription flow as a modal.
