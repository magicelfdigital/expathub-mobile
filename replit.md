...
- **Authentication**: `AuthContext` (`contexts/AuthContext.tsx`) manages JWT-based auth with external API at `expathub.world`. Token stored via `expo-secure-store` (iOS/Android) or `AsyncStorage` (web). Auth screen at `app/auth.tsx` (login/register modal). Account screen at `app/account.tsx` shows profile, subscription status, and logout. Profile icon in tab header and home screen top-right navigates to auth/account.
