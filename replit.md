...
- **One-command runner:** `npm run test:all` runs the full v1.4 suite end-to-end — Jest, then the web SPA Playwright specs (boots `npx vite build` + Express on port 5000, runs `locked-section` and `cancellation-exit-offer`, shuts the server down), then the Expo-web worksheet spec (boots `npx expo start --web --port 8081`, runs `worksheet-signup-submit`, shuts it down). Mirrors the `jest.yml` + `playwright.yml` CI jobs. Phase logs land in `server.log` and `expo.log`; a non-zero exit from any phase fails the overall run.
