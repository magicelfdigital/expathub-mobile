...
- **Authentication**: `AuthContext` (`contexts/AuthContext.tsx`) manages JWT-based auth with external APIs. Login uses `expathub.world/api/auth` (action: "signin"), registration uses `expathub.website/api/auth/register`. These are separate servers — accounts must exist on `expathub.world` for login to work. Token stored via `expo-secure-store` (iOS/Android) or `AsyncStorage` (web). Auth screen at `app/auth.tsx` (login/register modal). Account screen at `app/account.tsx` shows profile, subscription status, and logout. Profile icon in tab header and home screen top-right navigates to auth/account.
...
- **Real-time Updates**: Customer info listener in `EntitlementContext` automatically updates subscription state when purchases change, without requiring manual refresh.
...
- **Local Storage**: Decision Pass purchase timestamp stored in AsyncStorage (`decision_pass_purchased_at`). Country unlocks stored as array of slugs (`country_lifetime_unlocks`). Both synced with RevenueCat entitlements on refresh.
...
- **ProPaywall**: Custom 3-tier paywall component (`src/components/ProPaywall.tsx`) showing Decision Pass as primary, country unlock when country context is present, monthly subscription as backup. Contextual value propositions, sandbox testing toggle, and purchase/restore flows.
